...
class SmartContract {
   ...
   */
    ...
    connect(signer) {
        return __awaiter(this, void 0, void 0, function* () {
            /*
            const isAuthenticated = yield signer.isAuthenticated();
            if (!isAuthenticated) {
                throw new Error("the signer is unauthenticated");
            }
            */
            this._signer = signer;
    ...
    */
     ...
     * @param options An optional parameter that can specify the change address and additional UTXOs
    ...
    deploy(amount = 1, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const tx = yield this.buildDeployTransaction((options === null || options === void 0 ? void 0 : options.utxos) || [], amount, changeAddress);
    ...
    */
    ...
    /** @ignore */
    //Acrescentar address
    static signMultiCallTx(feeSigner, tx, address, calllogs, options) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            const instances = calllogs.map(log => log.instance);
            //Remover Address
            //const address = yield feeSigner.getDefaultAddress();
            // build sig requests for all inputs of transaction
            const network = feeSigner.provider.getNetwork();
            const contractInputs = tx.inputs.map((input, inputIndex) => ({
                input,
                inputIndex,
                instance: instances[inputIndex],
                calllog: calllogs[inputIndex],
            })).filter(item => item.instance instanceof SmartContract);
            const contractsUnlockingScripts = [];
            for (const item of contractInputs) {
                const { input, calllog, instance, inputIndex } = item;
                const pubKeyOrAddrToSign = calllog.pubKeyOrAddrToSign;
                const sigOptions = pubKeyOrAddrToSign ? (0, utils_1.parseSignatureOption)(pubKeyOrAddrToSign, network) : [{
                        address,
                        sigHashType: scryptlib_1.DEFAULT_SIGHASH_TYPE,
                    }];
                const sigRequests = sigOptions.map(item => {
                    return {
                        prevTxId: (0, scryptlib_1.toHex)(input.prevTxId),
                        outputIndex: input.outputIndex,
                ...
                // Skip if no signature is needed for call.
                let sigResponses = [];
                if (calllog.sigArgs.size > 0) {
                    sigResponses = yield instance.signer.getSignatures(tx.toString(), sigRequests);
                }
                contractsUnlockingScripts.push({
                    unlockingScript: this.handleSigResponses(sigResponses, instance, calllog, inputIndex),
                    inputIndex: item.inputIndex
                });
            }
            contractsUnlockingScripts.forEach(u => {
                tx.inputs[u.inputIndex].setScript(u.unlockingScript);
            });
            // If autoPayFee is false, then only sign contract call input.
            // Else, sign ALL inputs.
            ...
            const p2pkhInputs = tx.inputs.map((input, inputIndex) => ({
                input,
                inputIndex,
            })).filter(item => typeof instances[item.inputIndex] === 'undefined');
            const sigRequests = p2pkhInputs.map(item => {
                const { input, inputIndex } = item;
                return {
                    prevTxId: (0, scryptlib_1.toHex)(input.prevTxId),
                    outputIndex: input.outputIndex,
                    inputIndex: inputIndex,
            ...
            const sigResponses = yield feeSigner.getSignatures(tx.toString(), sigRequests);
            sigResponses.forEach(sigResp => {
                tx.applySignature({
                    inputIndex: sigResp.inputIndex,
                    sigtype: sigResp.sigHashType || scryptlib_1.DEFAULT_SIGHASH_TYPE,
    ...
    /** @ignore */
    static handleSigResponses(sigResponses, instance, calllog, inputIndex) {
        let group = sigResponses
            // group by inputIndex
            .reduce((group, sigResp) => {
            const inputIdx = sigResp.inputIndex;
            const sigs = group.get(inputIdx) || [];
        ...
        let methodArgs_ = calllog.methodArgs.map((arg, argIdx) => {
            const sigArg = calllog.sigArgs.get(argIdx);
            if (!sigArg) {
                return arg;
            }
            const validateSig = (sig) => {
            ...
            const sigs = sigArg.callback.call(this, group.get(inputIndex) || []) || '';
            if (sigs instanceof Array) {
                if (sigs.length !== sigArg.length) {
                    throw new Error(`the ${argIdx + 1}th argument calback function should returns a valid \`Sig\` array whose length is expected to be ${sigArg.length}, but got ${sigs.length}`);
                }
        ...
        if (calllog.exec) {
            instance.getUnlockingScript(self => {
                self[calllog.methodName](...methodArgs_);
            });
        }
        const { publicMethodCall } = instance.callDelegatedMethod(calllog.methodName, ...methodArgs_);
        return publicMethodCall.toScript();
    }
    /**
    ...
    * @param feeSigner a signer to sign the transation.
    * @returns a `MultiContractTransation` with a signed transation.
    */
    static multiContractCall(partialContractTx, feeSigner, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const address = yield feeSigner.getDefaultAddress();
            const feePerKb = yield feeSigner.provider.getFeePerKb();
            tx.feePerKb(feePerKb);
            // Add dummy data to input in order to get the fee estimate right.
            ...
            if (autoPayFee) {
                ...
                yield this.autoPayfee(tx, feePerKb, feeSigner, address, hasPrevouts ? 36 * calllogs.length : 0);
                if (hasPrevouts) {
                    this.dummySignMultiCallTx(tx, calllogs);
                    // Only adjust change out value.
                    tx['_feePerKb'] = feePerKb;
            ...
            yield this.signMultiCallTx(feeSigner, tx, address, calllogs, options);
            if (!options.partiallySigned) {
                ...
                yield feeSigner.provider.sendTransaction(tx);
            }
    ...
    static autoPayfee(tx, feePerKb, signer, address, additional = 0) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // todo fix bsv type
            let estimateSize = tx.getEstimateSize();
