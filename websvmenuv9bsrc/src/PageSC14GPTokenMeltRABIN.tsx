...
import { DefaultProvider, MethodCallOptions, sha256, toHex, PubKey, bsv, TestWallet, Tx, toByteString, hash160, buildPublicKeyHashScript, findSig, SignatureResponse, PubKeyHash, reverseByteString, int2ByteString } from "scrypt-ts";
  ...
  const interact = async (amount: any) => {
    ...
    if(txid.current.value.length === 64 || txidPrvStt.current.value.length === 64)
    {
      ...
      ////////////////////////////////////////////////////////
        ...
        console.log('PreVouts: ', toHex(tx.prevouts()))
        //console.log('Outputs: ', (reverseByteString(int2ByteString(BigInt(tx.outputs[0].satoshis), 8n), 8n) + scriptUxtoSize(tx.outputs[0].script.toHex()) + tx.outputs[0].script.toHex()))
        console.log('Outputs: ', (int2ByteString(BigInt(tx.outputs[0].satoshis), 8n) + scriptUxtoSize(tx.outputs[0].script.toHex()) + tx.outputs[0].script.toHex()))
        ...
        const s = '0x' + Buffer.from(certSig, 'hex').reverse().toString('hex')
        //const s = '0x' + Buffer.from(testFalhaSig, 'hex').reverse().toString('hex')
        let rSig: RabinSig
  
        //rSig = {s: BigInt(s), padding: ratesData.signatures.rabin.padding}
        rSig = {s: BigInt(s), padding: certPad}
        //rSig = {s: BigInt(s), padding: testFalhaPad}
  
      
        const { tx: callTx } = await instance2.methods.setupToken(rSig,
