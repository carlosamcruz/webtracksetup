...
export class GeneralTokenV2 extends SmartContract {
    ...
    //////////////////////////////////////////////////////////////////
    // Solução para evitar uso de UTXO criado sem um estado anterior do contrato: 
    ...
    public setupToken(
        ...
        if(len(this.prevUtxo) === 36n)
        {
            //Tenta reconstruir o TXID da transação com o UTXO que está sendo utilizado
            //  que deve conter o (TXID e Output Index) da transação que gerou o UTXO que esta sendo utilizado
            //  caso este UTXO seja uma replica do original, o TXID será diferente daquele apresentado para ser utilizado
            txidPrv = hash256(tx1P1 + this.prevUtxo + tx1P2 + UTXO1 + tx1P5)
        }
        
        else//if(len(this.prevUtxo) === 72n)
        {
            //Tenta reconstruir o TXID da transação com o UTXO que está sendo utilizado
            //  ***neste caso vindo de uma operação de Merge
            //  que deve conter os dois (TXIDs e Output Indexes) das transações que geraram o UTXO que esta sendo utilizado
            //  caso este UTXO seja uma replica do original, o TXID será diferente daquele apresentado para ser utilizado
            txidPrv = hash256(tx1P1 + slice(this.prevUtxo, 0n, 36n) + tx1P2 
            + slice(this.prevUtxo, 36n, 72n) + tx1P3 + UTXO1 + tx1P5)
        }
        /*
        if(len(this.prevUtxo) === 108n)
        {
            txidPrv = hash256(tx1P1 + slice(this.prevUtxo, 0n, 36n) + tx1P2 
            + slice(this.prevUtxo, 36n, 72n) + tx1P3 + slice(this.prevUtxo, 72n, 108n) + tx1P4 + UTXO1 + tx1P5)
        }
        */
    ...
    public mergeTokens(sig: Sig, pubkey: PubKey, 
    ...
    //public mergeTokens(sig: Sig, Supply1: bigint, Supply2: bigint) {    
        ...
        */
        ...
        //Jesus is the Lord!!!
        //
        //A entrada desta tx pode ter 2 inputs, e a saída 2 outputs

        if(len(this.prevouts) > 72n)
            this.prevUtxo = slice(this.prevouts, 0n, 72n) //2 output de cada vez
        else
            this.prevUtxo = slice(this.prevouts, 0n, 36n) //somente 1 output de cada vez
