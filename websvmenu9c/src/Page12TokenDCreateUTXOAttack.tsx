...
const Page12TokenDCreate: FC<props1> = (props) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        if(e.target)
        {
          ...
          const hexString = convertBinaryToHexString2(binaryString);
  ...
  const convertBinaryToHexString2 = (binaryString: any) => {
    const bytes = [];
    for (let i = 0; i < binaryString.length; i++) {
      const byte = binaryString.charAt(i);
      bytes.push(byte);
    }
    return bytes.join('');
  };
  ...
  */
  ...
  const writeToChain = async (amount: any) => {
    ...
    else if((txtData.current.value === "" && hexStrFileData === "" ) || satsAmount.current.value < 1)
    {
      alert("Missing Data");
      setsendButton(true)
    ...
    else
    {
      ...
      let data = txtData.current.value
      if(hexStrFileData != '')
      {
        data = newData;
      }
      ...
      if(props.passedData === 'UTXOattack')
      {

        //utxo2Replay = bsv.Script.fromHex(txtData.current.value)
        utxo2Replay = bsv.Script.fromHex(data)
        scriptData = utxo2Replay.toASM()
        console.log("Hash Scritp: ", sha256(data))
        console.log("Hash Scritp: ", sha256(toHex(utxo2Replay.toHex())))

        //Errado
        console.log("Hash Scritp: ", sha256(toHex((bsv.Script.fromASM(scriptData)).toHex())))
      ...
      */
      ...
      tx2.addOutput(new bsv.Transaction.Output({
        ...
        script: bsv.Script.fromHex(data),       
      ...
      <div>
        <div style={{ display: 'inline-block', textAlign: 'center', paddingBottom: '20px' }}>
          <label style={{ fontSize: '14px', paddingBottom: '0px' }}  
              > 
                 {/* <input ref={localPvtKey} type="hex" name="PVTKEY1" min="1" defaultValue={'PVT KEY'} placeholder="hex" />*/}
                 <input ref={txtData} type="hex" name="PVTKEY1" min="1" placeholder="UTXO script (HEX)" />
              </label>     
          </div>
      </div>
