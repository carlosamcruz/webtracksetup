...
const Page08TXDidactic: FC = () => {
  ...
  */
  ...
  const [fileName, setfileName] = useState("000");
  ...
  const downloadBinaryFile = () => {
    ...
    a.download = 'DIDACTIC-'+ fileName + '.' + fileType; // Specify the desired file name with the correct extension
  ...
  const sendSats = async (amount: any) => {
    ...
    else if(addToSend.current.value === "" || satsAmount.current.value === 0 || messageOpt.current.value.length !== 64)
    {
      alert("Missing Data");
      setsendButton(true)
    ...
    else
    {
      ...
      kInHex = messageOpt.current.value;
      ...
      let txABC = new bsv.Transaction

      let txPI = new bsv.Transaction

      //txPI.kDet('7afdd8b7bc28e251653e9dce712c5d5a088f0ba52cf78cfe712c82335f34bc8d')
      //console.log('Inv K: ', tx.invN('7afdd8b7bc28e251653e9dce712c5d5a088f0ba52cf78cfe712c82335f34bc8d'))
      //console.log('Inv Kinv: ', tx.invN('881acce714215685951f0bd7ac005cc0a61167e3b6bf906a11e855c7ef24c4c7'))


      txPI.kDet(kInHex)
      console.log('Inv K: ', tx.invN(kInHex))
      console.log('Inv Kinv: ', tx.invN(tx.invN(kInHex)))

      txPI.ditacticRST()


      //console.log('PreImage EXT: ', txPI.preimageExt())
      ...
      */
      ...
      
      {
          ...
          feeTX = (Math.floor((toHex(tx2).length/2)*0.001) + 1)*100
          //feeTX = Math.floor((toHex(tx2).length/2)*0.001) + 225 // BTC TestNEt
      }
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      setfileName(reverseHexBytes(sha256(sha256(rawTX).toString()).toString()))

      settxb(true);

      console.log('PreImage EXT New: ', txABC.preimageExt())

      console.log('Sig R: ', txABC.sigR())
      console.log('Sig S: ', txABC.sigS())

      let preImage = txABC.preimageExt()
      let e = '';

      let i = 0
      let j = 0
      //while( i !== -1)
      while( preImage.length > 0)
      {
        i = preImage.indexOf(';')
        //e = preImage.substring(0, i)
        //e = preImage.substring(1, i)

        //let x = ''

        e =  e + 'e' + j + ' = sha256(sha26(Preimage'+ j + ')):\n' + (sha256(sha256(preImage.substring(1, i)).toString()).toString()) + '\n\n'
        //e = 'e' + j + ' = sha256(sha26(Preimage'+ j + ')):\n' + (sha256(sha256(x).toString()).toString()) + '\n\n'
        j ++;

        console.log('PreImage ABCD: \n', e)

        if(i !== preImage.length)
        {
          preImage = preImage.substring(i + 1, preImage.length)
        }
        else
        {
          preImage = ''
      ...
      '1/K = ' + tx.invN(kInHex) + '\n\n' +
      'Preimage:\n' + txABC.preimageExt() + '\n\n'+ e +
      //'e = sha256(sha26(Preimage)):\n' + (sha256(sha256(txABC.preimageExt()).toString()).toString()) + '\n\n'+
      'Sig R = ' + txABC.sigR() + '\n\n' +
      'Sig S = ' + txABC.sigS() + '\n\n'+
      'TX:\n' + rawTX + '\n\n'+
      'TXID:\n' + reverseHexBytes(sha256(sha256(rawTX).toString()).toString())


      //let dataTX = toHex(data0)

      let dataTX = toByteString(data0, true)
      ...
      console.log('Data Size Final: ', (dataTX.length)/2)

      
      const encoder = new TextEncoder();
      const data02 = encoder.encode(dataTX);
      ...
      <div>
        <div style={{ display: 'inline-block', textAlign: 'center', paddingBottom: '20px' }}>
          <label style={{ fontSize: '14px', paddingBottom: '5px' }}  
              > 
                 {/* <input ref={localPvtKey} type="hex" name="PVTKEY1" min="1" defaultValue={'PVT KEY'} placeholder="hex" />*/}
                 <input ref={messageOpt} type="text" name="PVTKEY1" min="1" placeholder="k (hex)" />
              </label>     
          </div>
      </div>
...
export default Page08TXDidactic;