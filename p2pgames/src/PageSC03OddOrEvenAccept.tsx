...
import { DefaultProvider, sha256, toHex, PubKey, bsv, TestWallet, Tx, toByteString, ByteString, hash256, hash160, buildPublicKeyHashScript, findSig, SignatureResponse, PubKeyHash } from "scrypt-ts";
import './App.css';
...
const PageSC03OddOrEvenAccept: FC<props1> = (props) => {
  ...
  const writeToChain = async (amount: any) => {
    ...
    else
    {
      ...
      nextInstance.optionP2 = BigInt(optionP2.current.value);
      nextInstance.player2Add = PubKeyHash(toHex(bsv.Address.fromPrivateKey(pvtkey).hashBuffer));
      //O timeout do P2 deve começar a contar no minimo a partir do proximo bloco
      nextInstance.timeOutP2 = BigInt(info[0].blocks + 1) + instance2.timeout;

      await instance2.connect(signer)

      instance2.bindTxBuilder('acceptGame', async function () {
        ...
        //O timeout do P2 deve começar a contar no minimo a partir do proximo bloco
        unsignedTx.setLockTime((info[0].blocks + 1) + delktaNBlocks);


        if(BigInt(info[0].blocks + 1) <= instance2.timeOutP1){
          
          let contractAmount = ((2 * balance) * 90) / 100; 
          ...
          unsignedTx.addOutput(new bsv.Transaction.Output({
            ...
            script: buildPublicKeyHashScript(instance2.platFormAdd),
            satoshis: 1 + (2 * balance) - contractAmount
          }))
          ...
          let contractAmount = (balance * 99) / 100; 
          ...
          unsignedTx.addOutput(new bsv.Transaction.Output({
            //script: buildPublicKeyHashScript(hash160(instance2.alice)),
            script: buildPublicKeyHashScript(instance2.platFormAdd),
            satoshis: 1 + balance - contractAmount
          }))
      ...
      const { tx: callTx } = await instance2.methods.acceptGame(
      ...
      <div>
        <div style={{ display: 'inline-block', textAlign: 'center', paddingBottom: '20px' }}>
            <label style={{ fontSize: '14px', paddingBottom: '5px' }}  
                > 
                  {/* <input ref={localPvtKey} type="hex" name="PVTKEY1" min="1" defaultValue={'PVT KEY'} placeholder="hex" />*/}
                  <input ref={optionP2} type="number" name="OptionNumber" min="1" placeholder="Number Option >= 0" />
                </label>     
            </div>
        </div>
