...
class Blockchain extends scrypt_ts_1.SmartContractLib {
    ...
    static txInBlock(txid, bh, merkleProof) {
        return merklePath_1.MerklePath.calcMerkleRoot(txid, merkleProof) == bh.merkleRoot;
    }
    // Is txid the last transaction in a block
    static lastTxInBlock(txid, bh, merkleProof) {
        let last = true;
        let root = txid;
        for (let i = 0; i < merklePath_1.MerklePath.DEPTH; i++) {
    ...
    static blockHeight(bh, coinbaseTx, merkleProof) {
        // Ensure coinbase it's in the block.
        (0, scrypt_ts_1.assert)(Blockchain.txInBlock((0, scrypt_ts_1.Sha256)((0, scrypt_ts_1.hash256)(coinbaseTx)), bh, merkleProof));
        // Ensure it's the coinbase.
        (0, scrypt_ts_1.assert)(merklePath_1.MerklePath.isCoinbase(merkleProof));
